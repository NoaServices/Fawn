...
var FINDONEANDUPDATE = constants.FINDONEANDUPDATE;
...
var Task = function() {
   ...
   * @param condition the condition or data for this update
   * @param data the data for this update
   * @param options explicit options or/and fields projection
   *
   * @returns {Task}
   */
  task.findOneAndUpdate = function(model, condition, data, options) {
    if (!data) {
      if (!validDoc(model)) throw new Error("Invalid doc");
    ...
    if (options) {
      ...
      if (fields) updateObj.fields = xcode(fields);
      if (opts) updateObj.options = opts;
    }
...
function getResolveFunc(step) {
  switch(step.type){
    ...
    case FINDONEANDUPDATE: return performFindOneAndUpdate;
...
/**
 * This function handles the findOneAndUpdate step, only for mongoose.
 *
 * @param step the update step
 * @param task the task which step belongs to
 * @param results array of results from previous operations
 ...
 * @returns {Promise|*}
 */
function performFindOneAndUpdate(step, task, results) {
  var db = mongoose.connection.db;

  return step.useMongoose
    ? goose.performFindOneAndUpdate(db, step, task, results)
    : native.nativeUpdate(db, step, task, results)
}
